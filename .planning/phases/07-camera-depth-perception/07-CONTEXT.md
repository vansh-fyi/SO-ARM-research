...
### Hardware procurement update (2026-09-05, mid-planning)
- User is purchasing an off-the-shelf overhead camera stand/mount (Amazon India, ASIN B0GR913KGY) rather than designing the custom-printed cradle described in D-03. Calibration (exact position/angle measurement) will happen once the user physically assembles it. This does not change Phase 7's software scope — D-01/D-04's "best-estimate now, revisit once real mount exists" plan already anticipated this; the deferred real-camera photo comparison (D-04) still applies once assembly is done.
