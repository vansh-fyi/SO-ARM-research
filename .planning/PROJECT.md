...
- [X] Digital-twin fidelity: URDF rebuilt with correct kinematic chain (gripper attached to wrist end, not robot_base), `wrist_roll` + gripper jaw joints restored with correct axes/direction, in-repo relative mesh paths — Validated in Phase 10: Digital-Twin Fidelity (2026-09-19). Kinematics generated fresh from the already-correct MuJoCo model (not patched from the broken CoppeliaSim export); joint limits derived from live LeRobot calibration; gripper axis-direction mismatch found via mandatory hardware-in-the-loop re-test (TWIN-07 initially FAILED), fixed via gap-closure (flipped `gripper_left`/`gripper_right` polarity), and confirmed PASS on a second real-hardware re-test. A collateral regression in `collector.py`'s `OPEN_CMD`/`CLOSE_CMD` constants (paused Phase 8/9 scripted-grasp FSM) was caught by re-verification and fixed in the same session.
- [X] MuJoCo XML (`LIBERO/libero/libero/assets/robots/soarm101/robot.xml`) brought into agreement with the corrected URDF's kinematics — Validated in Phase 10 (TWIN-06): `env.reset()` regression suite green throughout.
- [X] An SO-101-native joint-action VLA (SmolVLA) connected to the real robot over the `control/` LeRobot serial bridge, with the action contract (units, joint order, gripper scale) and the degrees-vs-normalized ambiguity resolved — Validated in Phase 11 (VLAHW-01/02): `victorvanhalst/smolvla_so101_cube` drives the real SO-ARM101 over a Colab-hosted PolicyServer + ngrok tunnel via `control/vla_bridge`; `action_contract.py` documents units/joint order/gripper scale explicitly.
- [X] Safety validator gating every VLA action before it reaches the servos (joint limits, max displacement/velocity, gripper bounds, stale/malformed/NaN rejection, e-stop, comms-failure handling) — Validated in Phase 11 (VLAHW-02): `safety_validator.py` sits between every `pop_validated_action()`/`BridgeActionSource` call and `robot.send_action()`; absolute joint-limit clamping and NaN/inf rejection are untouched even after per-step/velocity/staleness caps were intentionally loosened for the live test session. No new hand-written raw serial/register code was added.
- [X] Harness capturing every inference step's complete I/O (frames, joint state, instruction, raw/validated/executed action, latency, model version) — Validated in Phase 11 (VLAHW-03): `io_logger.py`'s `episode.jsonl` confirmed to contain every required field on all 60 steps of the real recorded episode; per D-05, this is the correct substitute for a reasoning trace since SmolVLA (like other VLAs) produces none.
- [X] At least one full observed run of the VLA acting on the real robot, synced video + I/O log + termination reason recorded — Validated in Phase 11 (VLAHW-04/05): `control/outputs/11-05-retry-20260924-120530/` (60/60 steps, `termination.json` reason `max_steps_reached`, one confirmed real executed VLA action visibly moving the physical arm). Go/no-go findings in `control/vla_bridge/FINDINGS.md` (human-approved 2026-09-24): **GO**, conditional on a scoped follow-up fix for the observation-resend-per-tick latency issue that limited this episode's real-action yield to 1/60 — flagged by the human as needing further design brainstorming before implementation, not yet a settled fix.
...
None — all four v2.0 target requirements (SmolVLA hardware connection, safety validator, I/O harness, full observed run) validated in Phase 11 (see Validated section above). Milestone v2.0's two committed workstreams (Phase 10 digital-twin fix + Phase 11 VLA hardware connection) are both complete. Next milestone scope (deep-reasoning MLLM comparison, raw-autonomy design, or the tick-latency fix flagged in FINDINGS.md) is pending user decision — see Future Requirements below.
...
| Loosen `safety_validator.py`'s per-step/velocity/staleness caps ~8-10x for the Phase 11 live test session (Phase 11, 2026-09-24) | The physically empty test environment made safety margin a lower priority than achieving any real VLA-validated motion at all; original caps (5deg/step, 1.0s/3.0s staleness) discarded/clamped every real Colab-bridged action into invisibility given the tunnel's real round-trip latency | ✓ Committed (`d8c2785`/`8e2060a`); absolute joint-limit clamping and NaN/inf rejection left untouched |
| Fix `pop_validated_action()`'s `.pos`-suffixed vs plain-name key mismatch by normalizing at the bridge adapter boundary, not by changing `safety_validator`'s plain-name convention (Phase 11, 2026-09-24) | Plain names are the established convention throughout `action_contract.py` and the rest of the bridge; `.pos`-suffixing is `lerobot`'s own internal wire format, local to that one adapter boundary | ✓ Committed (`a3e16c4`/`8e2060a`) — this was the root cause silently emptying every real VLA action and crashing the first live episode |
| Phase 11 go/no-go: GO, conditional on fixing the observation-resend-per-tick latency issue before the next live episode (Phase 11, 2026-09-24) | Live episode confirmed the full pipeline works end-to-end (real action executed, safety-validated, visibly moved the robot) but yielded only 1/60 real actions because `control_loop_observation()` re-sends a full camera frame to Colab on every tick even while just draining an already-fetched chunk, inflating real tick time to ~11-20s against a configured 0.5s | ✓ Committed (human-approved with caveat: proposed fix needs further design brainstorming, not yet a settled implementation plan) |
...
*Last updated: 2026-09-24 — Phase 11 (VLA Hardware Connection) complete: milestone v2.0 is now 100% complete (both Phase 10's digital-twin fix and Phase 11's real-hardware VLA connection are done). A real, Colab-hosted SmolVLA checkpoint (`victorvanhalst/smolvla_so101_cube`) drove the physical SO-ARM101 over `control/vla_bridge`'s Colab/ngrok bridge, behind a live safety validator, with a genuine executed action confirmed visibly moving the robot. Three real bridge bugs (missing action-receiver thread, wrong staleness constant, `.pos`-suffix key mismatch silently emptying every validated action) were found and fixed live. `control/vla_bridge/FINDINGS.md` recommends GO on the next milestone phases, conditional on a scoped follow-up fix for a diagnosed tick-latency issue (observation re-sent to Colab every tick even while draining an already-fetched action chunk, ~11-20s/tick against a configured 0.5s) that limited the test episode to 1/60 real-action yield — the human flagged this proposed fix as needing further design before implementation. Next milestone scope (the tick-latency fix, deep-reasoning MLLM comparison, or raw-autonomy design per Future Requirements) is an open decision, not yet planned. Paused milestone v1.1 (Perception Fidelity & Checkpoint Benchmark) remains paused after Phase 7 completion; Phases 8-9 remain defined but not executed. Note for future phases: SO-ARM101 is a small ~500g-payload arm — tasks must keep objects (<=84mm) and targets within ~0.45m reach.*
